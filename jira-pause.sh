#!/bin/bash
# Moves ENG active-sprint tickets labelled "paused" -> Hold.
# Only touches the ENG board / current sprint. Env: DRY_RUN=1.
set -euo pipefail

source "$HOME/.config/jira-sweep.env"
TARGET_STATUS="Hold"
JQL='project = ENG AND sprint in openSprints() AND labels = paused AND status != "Hold"'
LOG="${JIRA_PAUSE_LOG:-$HOME/Library/Logs/jira-pause.log}"
DRY_RUN="${DRY_RUN:-0}"
